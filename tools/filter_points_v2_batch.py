...
def rotate_point(point, center, angle):
    ...
    # 将点移回原点
    rotated_point += np.array(center)[:,np.newaxis]
...
def is_point_in_box(points, box_params):
    ...
    # for i, point in enumerate(points):
        ...
        # 可以优化一下，一次处理一批点
    rotated_point = rotate_point(points[:,0:2], [x_center, y_center], yaw)
    
    # 检查点是否在框内
    mask = (abs(rotated_point[0,:] - x_center) <= len_x / 2) & \
        (abs(rotated_point[1,:] - y_center) <= len_y / 2) & \
        (0 <= np.array(points[:,2] - z_center)) & (np.array(points[:,2] - z_center) <= len_z)
# np.concatenate
