...
def load_pcd(file_path):
    """读取点云文件"""
    # data = np.fromfile(file_path, dtype=np.float32).reshape(-1, 5)[:,[0,1,2,3]]  # 读取XYZI点云
    points = pypcd.PointCloud.from_path(file_path)
    ...
    # data = np.frombuffer(pts_bytes, dtype=np.float32).reshape(-1, 5)[:,[0,1,2,3]]
    pcd = o3d.geometry.PointCloud()
    pcd.points = o3d.utility.Vector3dVector(points[:, :3])  # 仅使用XYZ坐标
    return pcd
...
def vis_from_path(path):
    ...
    bin_files = glob.glob(os.path.join(path, '*.npy'))
    # 遍历找到的文件
...
def vis_from_single(pcd_path,pred_path):
...
def main():
    # 指定路径
    pcd_path = '/media/seu/2T/points_cloud/pcd-64/1714248107.180461312.pcd'
    pred_path = '/media/seu/2T/points_cloud/pcd-64/1714248107.180461312_pred.npy'
    # vis_from_path(pcd_path,pred_path)
    vis_from_single(pcd_path,pred_path)
